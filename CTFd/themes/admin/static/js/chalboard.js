...
function loadchals(){
    ...
    $.post(script_root + "/admin/chals", {
        ...
        for (var i = 0; i <= challenges['game'].length - 1; i++) {
            ...
            var chal_button = $('<button class="chal-button col-md-2 theme-background" value="{0}"><h5>{1}</h5><p class="chal-points">{2}</p><span class="chal-percent">{3} team solved</span></button>'.format(chal.id, chal.name, chal.value, chal.percentage_solved));
